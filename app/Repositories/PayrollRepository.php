                ...
                if ($deduction->limit_per_month != 0 && $percentageOfDeduction >= $deduction->limit_per_month) {
                  ...
                  if($temp['deduction_name']=="NHIF")
                  {
                      $result = $this->calculateEmployeeAllowance($basic_salary, $pay_grade_id);
                         $nhifAmount  = $basic_salary - $result['totalAllowance'];
                        $nhif=0.03*($nhifAmount);
                         if($nhif<=20000)
              					 {
                           $nhif=20000;
              						      }
                    $temp['amount_of_deduction'] =  $nhif;
                  }elseif ($temp['deduction_name']=="PAYE") {
// $taxSalary=($gross_salary)-$nssf;
$result = $this->calculateEmployeeAllowance($basic_salary, $pay_grade_id);
   // $nhifAmount  = $basic_salary - $result['totalAllowance'];
$nssfAmount  = $basic_salary - $result['totalAllowance'];
   //$nhif=0.03*($nhifAmount);
$taxSalary=$basic_salary - 0.1*($basic_salary);
if($taxSalary<=270000){
  $payee =0;
}elseif($taxSalary>=27000 && $taxSalary<=520000){
    $payee = ($taxSalary-270000)*(8/100);

}elseif($taxSalary>=520000 && $taxSalary<=760000){
  $payee = ($taxSalary-520000)*(20/100)+20000;

}elseif($taxSalary>=760000 && $taxSalary<=1000000){
$payee = ($taxSalary-760000)*(25/100)+68000;
  ...
  $temp['amount_of_deduction'] =$payee;
                  }
